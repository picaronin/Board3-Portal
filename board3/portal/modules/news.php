			...
			if ($news < 0)
			{
				...
				for ($i = 0; $i < $count; $i++)
				{
					...
					$this->template->assign_block_vars('news.news_row', array(
						...
						'POST_ICON_IMG'			=> (!empty($fetch_news[$i]['icon_id'])) ? $icons[$fetch_news[$i]['icon_id']]['img'] : '',
						'POST_ICON_IMG_WIDTH'	=> (!empty($fetch_news[$i]['icon_id'])) ? $icons[$fetch_news[$i]['icon_id']]['width'] : '',
						'POST_ICON_IMG_HEIGHT'	=> (!empty($fetch_news[$i]['icon_id'])) ? $icons[$fetch_news[$i]['icon_id']]['height'] : '',
						'POST_ICON_IMG_ALT'		=> (!empty($fetch_news[$i]['icon_id'])) ? $icons[$fetch_news[$i]['icon_id']]['alt'] : '',
