...
class leaders extends module_base
{
	...
	*/
	...
	{
		// Display a listing of board users
		$this->user->add_lang('groups');
		$order_legend = ($this->config['legend_sort_groupname']) ? 'group_name' : 'group_legend';

        $selecteds = $groups_name = array();
        $selecteds = explode(';', $this->config['board3_leaders_groups_' . $module_id]);
        foreach ($selecteds as $selected)
        {
            $sql = 'SELECT group_id, group_name
                    FROM ' . GROUPS_TABLE . '
                    WHERE group_id = ' . (int)$selected;
            ...
            {
                $groups_name[] = $module_data['group_name'];
            }
        }

		if (!$this->config['board3_leaders_ext_' . $module_id] && sizeof($groups_name))
		{
			$legends = array();
			$groups = array();
			...
			if ($this->auth->acl_gets('a_group', 'a_groupadd', 'a_groupdel'))
			{
				$sql = 'SELECT group_id, group_name, group_colour, group_type, group_legend
					FROM ' . GROUPS_TABLE . '
					WHERE ' . $this->db->sql_in_set('group_name', $groups_name) . '
			...
			else
			{
				$sql = 'SELECT g.group_id, g.group_name, g.group_colour, g.group_type, g.group_legend
					...
					WHERE ' . $this->db->sql_in_set('g.group_name', $groups_name) . '
						AND (g.group_type <> ' . GROUP_HIDDEN . ' OR ug.user_id = ' . $this->user->data['user_id'] . ')
					ORDER BY g.' . $order_legend . ' ASC';
			}
			$result = $this->db->sql_query($sql, 150);
	...
	public function get_template_acp($module_id)
	{
		return array(
			...
			'vars'	=> array(
				...
				'board3_leaders_groups_' . $module_id	=> array('lang' => 'PORTAL_LEADERS_GROUP', 'validate' => 'string', 'type' => 'custom', 'method' => array('board3.portal.modules_helper', 'board3_leaders_groups_select'), 'submit' => array('board3.portal.modules_helper', 'board3_leaders_groups_submit'), 'explain' => true),
			),
		);
	}
	...
	*/
	public function install($module_id)
	{
		...
		$this->config->set('board3_leaders_groups_' . $module_id, '');
		return true;
	}
	...
	*/
	public function uninstall($module_id, $db)
	{
		$del_config = array(
			...
			'board3_leaders_groups_' . $module_id,
		);
